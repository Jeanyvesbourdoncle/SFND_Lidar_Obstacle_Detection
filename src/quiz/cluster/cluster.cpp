...
// Method for rendering our 2D tree
...
// call recursively, the method calling itself
void clusterHelper (int indice, const std::vector<std::vector<float>> points, std::vector <int>& cluster, std::vector<bool>& processed, KdTree* tree, float distanceTol) 
{
	...
	// with indice, we reference it  from the dataset of 11 points
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	// iteration through those 11 points
	int i=0;
	while (i < points.size())
	{
		// if the point has been processed already, we make an incrementation (move on the next point)
		if (processed[i])
		{
			i++;
			continue;
		...
		// if not, a brand new cluster and the cluster is represented by this vector of ints
		std::vector <int> cluster;
		// clusterHelper = function proximity 
		// pointid, points, cluster passed as reference, which points have been processed or not, tree= eucledian clustering (nearby neighbor search)/search fonction, distance tolerance 
		clusterHelper (i, points, cluster, processed, tree, distanceTol);
		clusters.push_back(cluster); // cluster push back into clusters list
		i++; // move to the next points
	}
	return clusters;
...
int main ()
{
	...
	// create the kdtree and we insert our points (same points that we were looking at earlier for our 2D data)
	KdTree* tree = new KdTree;
  
    for (int i=0; i<points.size(); i++) 
    	tree->insert(points[i],i); 

	// call the rendered 2D tree
  	int it = 0;
